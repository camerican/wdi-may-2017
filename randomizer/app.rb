...
def initialize
    @roster = %w(Hans James Victor Nick Neil Dennis Logan Erin Steven Julia Katy)
    @past_groups = [%w(Hans James),
      %w(Victor Nick),
      %w(Neil Logan),
      %w(Erin Steven),
      %w(Julia Katy)
    ...
    if @@mode == :v1
      ...
      for i in 0...@roster.length 
        ...
        if group.length == size
          groups.push group
          color = @@colors.sample
          puts Rainbow("Round #{(i+2)/2} (#{color})").color(color.to_sym)
          group = [] # reset group to empty array reference
        end
        ...
        puts Rainbow("Current Group \##{groups.length+1}: ").color(color.to_sym) + Rainbow(group.join(', ')).red
        sleep(3)
      end
      # after the for loop concludes, determine whether we have enough
      # people in the last group to add them, or whether to break them
